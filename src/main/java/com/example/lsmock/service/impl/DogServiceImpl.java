...
public class DogServiceImpl implements DogService {
    ...
    public String sendMessage(String title, String content, String url, List<String> pins) throws Exception{
        ...
        params.add(new BasicNameValuePair("accessToken", findDogToken().getAccessToken()));//授权后获得
        params.add(new BasicNameValuePair("aspid", findDogToken().getAspid()));//需要申请
        params.add(new BasicNameValuePair("version", DOG_version));
        ...
        json.put("type", findDogCfg().getType());
        json.put("ver", findDogCfg().getVer());
        json.put("title", title);//长度限制50
        json.put("content", content);//目前暂不支持换行，长度限制1500
        json.put("noticeId", findDogCfg().getNoticeId());// ~加原来的应用标识  如原来的 ump -> ~ump，申请邮件反馈中会给出相应的noticeId
        json.put("toTerminal", findDogCfg().getToTerminal());//企业版支持(推所有终端传 7 )
        json.put("app", findDogCfg().getApp());//代表发送的用户群体，ee代表国内ERP，泰国th.ee
